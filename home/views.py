    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request, format_html('one item added to cart ... <a href="http:/3.86.161.235/cart">view cart</a>'))
                return redirect('home')
            else:
                newitem = Cart()
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_34ca5572e38bc0be723e0be85699b2a85e9b96d9' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url 
        cburl = 'http:/3.86.161.235/callback' #payment thank you page 
        ...
        order_no = profile.id 
        total = float(request.POST['total']) * 100 #total amount to be charged from the user card
